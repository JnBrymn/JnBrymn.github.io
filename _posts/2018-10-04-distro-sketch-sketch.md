...
Finally, the cost function that controls which point is removed is pretty generic (refer back to _[The Algorithm]({% post_url 2018-10-04-distro-sketch-sketch %}#the-algorithm)_ section). We can modify the behavior significantly by penalizing distance between points differently than point increments. We can also add in other features to the cost function, such as how long the point has existed.
